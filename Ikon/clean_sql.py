...
@profile
def find_artist_variants(connection, searchlimit=None):
    sql = '''
    SELECT id, name
    FROM artists
    ORDER BY name ASC;
...
if __name__ == "__main__":
    connection = connectToDatabase("./data/ikon_clean2.db")
    ...
    # find variants and merge them
    def merge_artists(connection, variants):
        sql = '''
    ...
    for variants in find_artist_variants(connection):
        for a in variants:
            print("-", a[1])
        print()
        merge_artists(connection, variants)
        
        
    from fuzzywuzzy import fuzz
    @profile
    def find_fuzzy_variants(connection, searchlimit=None):
        sql = '''
        SELECT name
        FROM artists
        ORDER BY name ASC;
        '''
        stack = set()
        all_artists = [_[0] for _ in connection.execute(sql)][:searchlimit]
        for A in all_artists:
            stack.add(A)
            variants = [A]
            for B in all_artists:
                if B not in stack and fuzz.ratio(A, B)>90:
                    variants.append(B)
                    stack.add(B)
    ...
    fuzzy_variants = []
    for variants in find_fuzzy_variants(connection):
        print(variants)
        fuzzy_variants.append(variants)
        merge_artists(connection, variants)
